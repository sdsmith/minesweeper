...
// Log priority level (each one being a subset of the messages from earlier
// levels)
...
#define logMessage(log_type, log_level, fmt, ...) \
    SDL_LogMessage((log_type), (log_level), (fmt), ##__VA_ARGS__)
#define log(fmt, ...) SDL_Log((fmt), ##__VA_ARGS__)
#define logVerbose(log_type, fmt, ...) \
    SDL_LogVerbose((log_type), (fmt), ##__VA_ARGS__)
#define logDebug(log_type, fmt, ...) \
    SDL_LogDebug((log_type), (fmt), ##__VA_ARGS__)
#define logInfo(log_type, fmt, ...) \
    SDL_LogInfo((log_type), (fmt), ##__VA_ARGS__)
#define logWarn(log_type, fmt, ...) \
    SDL_LogWarn((log_type), (fmt), ##__VA_ARGS__)
#define logError(log_type, fmt, ...) \
    SDL_LogError((log_type), (fmt), ##__VA_ARGS__)
#define logCritical(log_type, fmt, ...) \
    SDL_LogCritical((log_type), (fmt), ##__VA_ARGS__)
#define logSetPriority(log_type, log_level) \
    SDL_LogSetPriority((log_type), (log_level))
