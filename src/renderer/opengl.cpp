...
OpenGl::check_gl_error(std::string const &description, std::string const &file, std::size_t const line) {
	...
	logCritical(LOG_VIDEO,
                    "OpenGL Error: %s - %s\n\t%s, line %d\n",
                    log.c_str(), description.c_str(), file.c_str(), static_cast<int>(line));
...
OpenGl::proto_setup() {
    ...
    GL_CHECK(glBindVertexArray(vao));
    {
        ...
        GL_CHECK(glVertexAttribPointer(2, 2, GL_FLOAT, GL_FALSE, 8 * sizeof(GLfloat), reinterpret_cast<GLvoid*>(6 * sizeof(GLfloat))));
        GL_CHECK(glEnableVertexAttribArray(2));
        GL_CHECK(glBindBuffer(GL_ARRAY_BUFFER, 0)); // NOTE(sdsmith): vbo bound to vao after glVertexAttribPointer call
    }
...
//                       GLint wrapS, GLint wrapT, GLint minFilter, GLint maxFilter) {
...
OpenGl::set_vsync(bool enable) {
    ...
    if (GLXEW_EXT_swap_control) {
        ...
        // TODO(sdsmith): fall back on SDL. First, figure out a way to keep SDL decoupled
    }
}
