...
ShaderFile::~ShaderFile() { glDeleteShader(shader_id); }

GLuint ShaderFile::get_id() const { return shader_id; }

void ShaderFile::compile_shader(GLenum shader_type)
{
    // Get file content
    std::string code;
...
FragmentShader::FragmentShader(std::string const &file_path)
    : ShaderFile(file_path)
{
...
GLuint Shader::get_id() const { return program_id; }

Shader::~Shader() { GL_CHECK(glDeleteProgram(program_id)); }

void Shader::enable() { GL_CHECK(glUseProgram(program_id)); }

void Shader::disable() { GL_CHECK(glUseProgram(0)); }
